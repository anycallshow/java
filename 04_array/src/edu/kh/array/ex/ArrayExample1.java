...
public class ArrayExample1 {
	...
	/*
	 * 배열 (Array) - 같은 자료형의 변수를 하나의 묶음으로 다루는 것 (자료구조) - 묶어진 변수들은 하나의 배열명으로 불러지고 구분은
	...
	public void ex1() {
		...
		// ** 생선된 int[]에는 시작 주소가 지정된다!! **

		// arr = new int[3];
		// (int[]) (int[]) -> 같은 자료형 == 연산 가능

		// heap 영역에 생선된 int[]의 시작 주소를
	...
	// null : 참조형 변수가 선언 되었으나 아무것도 참조하지 않음
	// (빈칸 아님!!!)
	...
	public void ex3() {
		...
		// heap영역에 새로 생성된 double 5칸 짜리 double[]의 시작 주소를 height에 대입
	...
	public void ex5() {
		...
		for(int i=0; i < arr.length; i++) {
			...
			// B == 66
			// C == 67
			// char 자료형은 문자표에 저장된 번호를 불러옴
		}
		
		// ** Arrays 클래스
		// -> Java에서 제공하는 배열과 관련된 기능을 모아둔 클래스 
		int[] arr2 = new int[4];
		System.out.println(arr2); // [I@6f2b958e
	...
	public void ex8() {
		...
		String[] arr = {"사과", "딸기", "바나나", "키위", "멜론", "오렌지"};
		...
		for(int i=0; i < arr.length; i++) {
			...
			if(arr[i].equals(input)) { // 문자열 비교 : 문자열.equals(문자열)
				System.out.println(i + "번째 인덱스에 존재");
				flag = true;
			}
		}
