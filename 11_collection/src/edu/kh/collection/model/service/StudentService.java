...
public class StudentService {
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				menuNum = -1; // 첫 반복 시 잘못 입력하는 경우 menuNum이 0을 가지고 있어서 종료되는데
				// 이를 방지하기 위해 임의값 -1 대입
			}
	...
	public String addStudent() throws InputMismatchException{
		...
		char gendet = sc.next().toUpperCase().charAt(0);
		// String -> char
		...
		if(studentList.add( new Student(name, age, region, gendet, score) )) {
			// boolean java.util.List.add(Student e)
			// (반환형)						-> 제네렉 <Student> 때문에 List 타입이 Student로 제한
	...
	public void selectAll() {
		...
		if (studentList.isEmpty() ) {
			...
			return; // 현재 메소드 종료하고 호출한 곳으로 돌아감 
			// 단, 반환값은 없음(반환형이 void이기 때문에) 
		}
		/* 일반 for문
		...
		// (순차적 : 0번 인덱스부터 마지막 요소가지 인덱스를 1씩 증가)

		// [작성법] 
		// for( 컬렉션 또는 배열에선 꺼낸 한 개의 요소를 저장할 변수 : 컬렉션명 또는 배열명 ){}
		int index = 0;
	 ...
	 * 3. 회원 정보 수정 메소드
	 */
	public String updateMember() throws InputMismatchException{
		...
		if( studentList.isEmpty() ) {
			...
			// -> 반환된 값을 temp에 저장
			Student temp = studentList.set(index, new Student(name, age, region, gendet, score));
	...
	public String removeStudent() throws InputMismatchException{
		...
		if( studentList.isEmpty() ) {
			...
			// String -> 대문자 String -> 대문자 0번 인덱스 문자(첫 글자)
