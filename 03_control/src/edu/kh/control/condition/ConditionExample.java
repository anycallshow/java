...
public class ConditionExample {
	...
	public void ex3() {
		...
		if(input > 0) { // input이 양수일 경우
			...
			// 바로 위에 있는 if문이 만족되지 않은(false) 경우 수행 
	...
	public void ex5() {
		...
		}else if(age > 13 && age <= 19) { // else if ( age <= 19 ) 로 사용가능
	...
	public void ex8() {
		...
		if(age < 0 || age > 100) {
			...
			if(height < 0 || height > 250.0) {
				...
				if(age >= 12 && height < 140.0) { // 나이 O, 키 X
					result = "나이는 적절하나, 키가 적절치 않음";
					
				} else if(age < 12 && height >= 140.0) { // 나이 X, 키 O
