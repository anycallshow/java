...
public class ForExample {
	...
	public void ex7() {
		...
		// * Char 자료형은 문자형 이지만 실제로는 정수(문자표 번호)를 저장한다.ㅣ
...
//		System.out.println(input);  // 에러
	...
	public void ex14() {
		...
		if(dan>9 || 2>dan) { // 입력 받은 수가 2 ~ 9 사이 숫자가 아닐 경우
			System.out.println("잘못 입력 하셨습니다.");
		}else {				// 입력 받은 수가 2 ~ 9 사이 숫자일 경우
			for(int i = 1; i<=9; i++) {
	...
	public void ex15() {
		// [19단 출력기]
		// 2 ~ 19단 사이 단을 입력 받아서 x1 ~ x19 까지 출력
	...
	public void ex16() {
		...
		for(int dan = 2; dan<=9; dan++) { // 2 ~ 9단까지 차례로 증가
			...
			for(int num = 1; num<=9; num++) { // 각 단에 곱해질 수 1 ~ 9까지 차례대로 증가
				System.out.printf("%2d X %1d = %2d ", dan , num, dan * num);
	...
	public void ex19() {
		...
		for (int a = 4; a >= 1; a--) { // 줄 반복
			
			for(int b = a ; b >= 1; b --) { // 숫자 출력
				System.out.print(b);
